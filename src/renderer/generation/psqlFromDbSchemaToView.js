...
export default (sql) => {
  ...
  rawStmts.forEach((rawStmt) => {
    ...
    if (createStmt) {
      ...
      let columns = [];
      if (createStmt.inhRelations) {
        createStmt.inhRelations.forEach((inhRelation) => {
          const inhTable = result.tables.find(({name}) => name === inhRelation.RangeVar.relname);
          columns = JSON.parse(JSON.stringify(inhTable.columns));
        });
      }
