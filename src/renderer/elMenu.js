...
import psqlFromViewToDbSchema from './generation/fromViewToDbSchema.js';
...
//   _setSchema(schema);
// });
electron.ipcRenderer.on('export-sql', async (sender, filePath) => {
  let result;
  const schema = _getCurrentSchema();
...
electron.ipcRenderer.on('import-sql', async (sender, filePath) => {
  let result;
  const schema = _getCurrentSchema();
  switch (schema.dbType) {
    case 'psql':
